...
export const Projects = () => {
  ...
  return (
    <MainLayout style={"bg-[#1c1c1c]"}>
      <div className="w-full grid grid-rows-3 grid-flow-col gap-4 sm:grid sm:grid-cols-1 sm:grid-flow-row xl:grid xl:grid-cols-2 xl:grid-flow-col xl:gap-4 justify-center items-center pt-5 mx-auto portrait:grid portrait:grid-cols-1 portrait:grid-flow-row">
        {datas?.map((x, i) => (
          <div
            key={i}
          ...
          > <div className="w-full pl-9 cursor-pointer portrait:text-base portrait:pl-4">
            ...
            <div>Link: <a href={x?.link} target="_blank">{x?.link}</a></div>
            </div>
